<?php
// Archivo: includes/register_process.php
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (
        ...
        $correo = trim($_POST['correo']); // Limpiar correo
        $password = $_POST['password'];   // No modificar contraseña
        ...
        if (
            ...
            die("<p>Nombre y apellidos no pueden contener solo números. <a href='../public/register.php'>Volver</a></p>");
        }

        // Revisar que el correo tenga un formato válido
        if (!filter_var($correo, FILTER_VALIDATE_EMAIL)) {
            die("<p>Correo inválido. <a href='../public/register.php'>Volver</a></p>");
        }
        ...
        $stmt->execute([$correo]);
        if ($stmt->fetch()) {
            die("<p>El correo ya está registrado. <a href='../public/register.php'>Volver</a></p>");
        }
        ...
        try {
            ...
            die("<p>Error al registrar: " . $e->getMessage() . " <a href='../public/register.php'>Volver</a></p>");
        }

    } else {
        // Si faltan campos en el formulario
        die("<p>Faltan campos requeridos. <a href='../public/register.php'>Volver</a></p>");
    }
} else {
    // Si alguien accede directamente al archivo sin POST, redirigir
