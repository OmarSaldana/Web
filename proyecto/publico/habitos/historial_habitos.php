<?php
// Archivo: publico/habitos/historial_habitos.php
...
switch ($tipo) {
    case 'semana':
        // Año y semana actual
        $ajustar_consulta = "AND YEARWEEK(fecha, 1) = YEARWEEK(CURDATE(), 1)";
        break;
    case 'mes':
        // Año y mes actual
        $ajustar_consulta = "AND YEAR(fecha) = YEAR(CURDATE()) AND MONTH(fecha) = MONTH(CURDATE())";
        break;
    default:
        // Solo el día actual
        $ajustar_consulta = "AND fecha = CURDATE()";
        break;
}

// Consultar para traer habitos
$stmt = $pdo->prepare("SELECT h.nombre_habito, hh.fecha, hh.completado
    ...
    WHERE h.id_usuario = ? $ajustar_consulta
...
<head>
  ...
  <link rel="stylesheet" href="styles_habits/styles_historial_habitos.css">
</head>
<body>
