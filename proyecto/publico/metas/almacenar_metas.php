<?php
// Archivo: publico/metas/almacenar_metas.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_habito || !$descripcion || !$cantidad_objetivo || !$fecha_inicio || !$fecha_fin) {
        header("Location: crear_metas.php?error=faltan_datos");
        exit;
    }

    // Validar que la fecha de inicio no sea posterior a la de fin
    if (strtotime($fecha_inicio) > strtotime($fecha_fin)) {
        header("Location: crear_metas.php?error=fecha_invalida");
        exit;
    }
    ...
    header("Location: crear_metas.php");
    exit;
}
